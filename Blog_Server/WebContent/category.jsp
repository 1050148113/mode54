...
//判断有没有用户列表数据，如果没有这跳转到 user.s?op=query
	if(request.getAttribute("userlm")==null){
		request.getRequestDispatcher("user.s?op=cate").forward(request, response);
	}
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <div class="col-md-5">
          ...
          <form action="user.s" method="post" autocomplete="off">
            <input type="hidden" name="op" value="adda">
            <div class="form-group">
              <label for="category-name">栏目名称</label>
              <input type="text" id="category-name" name="columnName" value="${param.columnName}" class="form-control" 
            ...
            <div class="form-group">
              ...
              <input type="text" id="category-alias" name="aliasName" 
              	value="${param.aliasName}"	class="form-control" placeholder="在此处输入栏目别名" required autocomplete="off">
            ...
            <div class="form-group">
              ...
              <select id="category-fname" class="form-control" name="parentId">
                ...
                <option value="${param.parentId}">${u.columnName}</option>
               </c:forEach>
              </select>
            ...
            <div class="form-group">
              ...
              <input type="text" id="category-keywords" name="keyWords" value="${param.keyWords}" class="form-control" placeholder="在此处输入栏目关键字" autocomplete="off">
              <span class="prompt-text">关键字会出现在网页的keywords属性中。</span> 
            </div>
            <div class="form-group">
              <label for="category-describe">描述</label>
              <textarea class="form-control" id="category-describe" name="description" value="${param.description}" rows="4" autocomplete="off"></textarea>
        ...
        名称:<input name="columnName" value="${param.columnName}">
        别名:<input name="aliasName" value="${param.aliasName}">
        总数:<input name="status" value="${param.status}">
            ...
            <table class="table table-striped table-hover">
              ...
              <c:forEach items="${userlm}" var="u">
                <tr>
                  ...
                  <td>${u.aliasName}</td>
                  <td>${u.status}</td>
                  <td><a  href="category_update.jsp" rel="${u.id} name="see">修改</a> <a rel="${u.id }">删除</a></td>
                </tr>
        ...
        if (name.attr("name") === "see") {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#category_update.jsp').modal('show');
                }
